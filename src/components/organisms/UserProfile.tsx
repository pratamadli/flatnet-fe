...
import { Form, Button, Label } from "../atoms";
...
const UserProfile = () => {
  ...
  const [currentData, setCurrentData] = useState<CurrentUserDataPayload>();
  ...
  useEffect(() => {
    dispatch(getAuthThunk(user?.token || ""))
      .then((data) => {
        if (data.payload.success) {
          setCurrentData(data.payload.data);
